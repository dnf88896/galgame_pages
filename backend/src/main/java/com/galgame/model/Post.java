 ...
 * liked / disliked / favorited / replies 为 null 时序列化省略（列表接口不含这三项）；
...
public record Post(
    ...
    public static Post core(Long id, Long userId, String author, String category, List<String> tags,
                            ...
                            int replyCount, int viewCount, int likeCount, int dislikeCount, String coverImage,
                            LocalDateTime pinnedUntil) {
        return new Post(id, userId, author, category, tags, title, content, createdAt,
                replyCount, viewCount, likeCount, dislikeCount, coverImage, pinnedUntil,
    ...
    public Post withTags(List<String> tags) {
        return new Post(id, userId, author, category, tags, title, content, createdAt,
                replyCount, viewCount, likeCount, dislikeCount, coverImage, pinnedUntil,
                liked, disliked, favorited, attachments, replies);
    }

    /** 附加上下文（点赞/点踩/收藏状态 / 附件 / 回复）得到对外完整对象 */
    public Post withContext(Boolean liked, Boolean disliked, Boolean favorited,
                            List<Attachment> attachments, List<Reply> replies) {
        return new Post(id, userId, author, category, tags, title, content, createdAt,
                replyCount, viewCount, likeCount, dislikeCount, coverImage, pinnedUntil,
                liked, disliked, favorited, attachments, replies);
    }
