 ...
 * liked / replies 为 null 时序列化省略（列表接口不含这两项）；
...
public record Post(
    ...
    public static Post core(Long id, Long userId, String author, String category, List<String> tags,
        ...
        return new Post(id, userId, author, category, tags, title, content, createdAt,
                replyCount, viewCount, likeCount, null, null, null);
    }

    /** 附加标签数组（DAO 批量加载 tags 后调用），保留其余字段 */
    public Post withTags(List<String> tags) {
        return new Post(id, userId, author, category, tags, title, content, createdAt,
                replyCount, viewCount, likeCount, liked, attachments, replies);
    }

    /** 附加上下文（点赞状态 / 附件 / 回复）得到对外完整对象 */
    public Post withContext(Boolean liked, List<Attachment> attachments, List<Reply> replies) {
        return new Post(id, userId, author, category, tags, title, content, createdAt,
                replyCount, viewCount, likeCount, liked, attachments, replies);
    }
}
