<template>
  <div class="page">
    <div class="gal-center">
      <header class="gal-header">
        <div class="gal-header-row">
          ...
          <div class="gal-header-actions">
            ...
            <el-badge
              v-if="isAdmin"
              :value="reviewCount?.character ?? 0"
              :hidden="!isAdmin || !(reviewCount?.character > 0)"
              :max="99"
            >
              <button class="gal-review-btn" @click="router.push('/character/review')">审核角色</button>
            </el-badge>
            <button v-if="isLoggedIn" class="gal-mine-btn" @click="router.push('/character/mine')">我的提交</button>
            <button v-if="isLoggedIn" class="gal-add-btn" @click="router.push('/character/new')">+ 添加角色</button>
          </div>
        </div>
        <h1 class="gal-title">角色</h1>
        <p class="gal-subtitle">收录 Galgame 角色信息。按名称搜索，登录用户可提交，由管理员审核后上架。</p>
      </header>
      ...
      <!-- 列表 / 空状态 -->
      <template v-else>
        <div v-if="characters.length" class="gal-list">
          <article v-for="c in characters" :key="c.id" class="gal-card" @click="goDetail(c.id)">
            <div class="gal-cover">
              <img
              ...
              />
              <div v-else class="gal-cover-placeholder">角色</div>
            </div>
            <div class="gal-card-body">
              ...
              <div class="gal-views">
                ...
                <span v-if="c.created_at">创建：{{ formatTime(c.created_at) }}</span>
              </div>
            </div>
          </article>
        </div>
...
// 管理员（admin_level > 0）才显示「审核角色」入口；等级来自 store，认证后实时刷新
const isAdmin = computed(() => Number(user.value?.admin_level) > 0)

// 登录用户（有 id）才显示「添加角色」入口
...
async function load() {
  ...
  try {
    ...
    hasMore.value = characters.value.length >= PAGE_SIZE
  } catch (e) {
...
const emptyTitle = computed(() => (searchWord.value.trim() ? '没有符合条件的角色。' : '暂无角色资料'))
const emptyDesc = computed(() =>
  searchWord.value.trim() ? '' : isLoggedIn.value ? '点击右上角「添加角色」创建' : ''
)
