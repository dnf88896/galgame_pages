  ...
  <div class="page">
    <div class="review-center">
      <div class="review-head">
        <el-page-header content="审核角色" @back="goBack" />
      </div>
        ...
        <template v-else>
          <div v-if="pendingList.length" class="review-list">
            <article v-for="p in pendingList" :key="p.id" class="gal-card" @click="goDetail(p.id)">
              <div class="gal-cover">
                <div class="gal-cover-placeholder">角色</div>
              </div>
          ...
          <div v-else class="review-empty">
            <p class="review-empty-text">暂无待审核的角色</p>
          </div>
        </template>
      </template>
    </div>
...
// 管理员（admin_level > 0）才能看待审列表；等级来自 store，认证后实时刷新
...
async function load() {
  ...
  loadError.value = ''
  try {
    const { data } = await api.get('/characters/pending')
