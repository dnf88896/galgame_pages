  ...
  <div class="page">
    <div class="gal-detail-center">
      ...
      <div v-else-if="notFound" class="gal-detail-empty">
        <p class="gal-detail-empty-text">角色不存在</p>
        ...
        <template v-if="!editMode">
          ...
          <div class="gal-detail-card">
            <div class="gal-detail-cover">
              ...
              />
              <div v-else class="gal-detail-cover-placeholder">角色</div>
            </div>
          ...
          <!-- 关联制作人员：该角色关联的已上架制作人员（详情接口返回 staffs: [{id,name}]） -->
          <div class="works-section">
            <h2 class="works-title">关联制作人员</h2>
            <div v-if="staffs.length" class="relation-list">
              <a
                v-for="s in staffs"
                :key="s.id"
            ...
            <div v-else class="works-empty">
              <p class="works-empty-text">暂无关联制作人员</p>
            </div>
          </div>

          <!-- 作品列表：该角色已上架的 galgame（GET /api/galgames?character_id=X） -->
          <div class="works-section">
...
// 是否本条目的创建者（后端 characters.created_by，数字 id）
...
// 关联制作人员：详情接口返回 staffs: [{id,name}]（snake_case 键）
const staffs = computed(() => (Array.isArray(detail.value?.staffs) ? detail.value.staffs : []))

// 作品列表：GET /api/galgames?character_id=X 返回该角色已上架的 galgame（snake_case 键）
const works = ref([])
const worksLoading = ref(false)
const worksError = ref('')

// 关联制作人员下拉选项：GET /staffs 返回数组（不是 {data:...} 包一层），失败兜底空数组不影响编辑
...
// 作品列表：GET /api/galgames?character_id=X 返回该角色已上架的 galgame 数组
...
function goContributors() {
  router.push('/character/' + detail.value.id + '/contributors')
}

// 返回按钮：有站内上一页就真后退，否则兜底替换到下方目标页
// （判断依据与 replace 兜底的原因见 utils/navigation.js）
function goBack() {
  navGoBack(router, '/character')
}
...
// 进入编辑模式：用已加载详情数据回填表单（关联制作人员回填 id 数组）
function enterEdit() {
  ...
  form.staffIds = Array.isArray(d.staffs) ? d.staffs.map((s) => s.id) : []
  editMode.value = true
}
...
async function remove() {
  try {
    await ElMessageBox.confirm('确定删除该角色吗？删除后不可恢复。', '删除确认', { type: 'warning' })
  } catch {
    return // 用户取消，什么都不做
  }
  try {
...
// 通过审核：POST /characters/{id}/review { status: 'approved' }，成功后刷新详情。
// 提交者恰为当前用户（管理员审核自己的提交）时 +10，refreshMoe 检测增量弹「+n萌点」；审核他人无变化不弹。
async function approve() {
  try {
    await api.post(`/characters/${characterId}/review`, { status: 'approved' })
...
// 拒绝：弹框填理由 → POST /characters/{id}/review { status: 'rejected', reason }，成功后刷新详情
async function reject() {
  ...
  try {
    await api.post(`/characters/${characterId}/review`, { status: 'rejected', reason })
...
/* 关联制作人员链接（复用 gal-staff-link 蓝色链接样式） */
.relation-list {
  display: flex;
  flex-wrap: wrap;
  gap: 12px;
