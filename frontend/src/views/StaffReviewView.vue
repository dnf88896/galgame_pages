  ...
  <div class="page">
    <div class="review-center">
      <div class="review-head">
        <el-page-header content="审核制作人员" @back="goBack" />
      </div>
        ...
        <template v-else>
          <div v-if="pendingList.length" class="review-list">
            <article v-for="p in pendingList" :key="p.id" class="gal-card" @click="goDetail(p.id)">
              <div class="gal-cover">
                <div class="gal-cover-placeholder">制作人员</div>
              </div>
          ...
          <div v-else class="review-empty">
            <p class="review-empty-text">暂无待审核的制作人员</p>
          </div>
        </template>
      </template>
    </div>
...
async function load() {
  ...
  loadError.value = ''
  try {
    const { data } = await api.get('/staffs/pending')
